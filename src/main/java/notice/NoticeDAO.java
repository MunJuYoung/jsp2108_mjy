...
public class NoticeDAO {
	...
	// 게시글의 총 건수 구해오기
		...
		public int totRecCntLately(int lately) {
			int totRecCnt = 0;
			try {
				sql = "select count(*) from notice where date_sub(now(), interval ? day) > wDate";
				pstmt = conn.prepareStatement(sql);
		...
		// 최근 게시글 보기...
		public List<NoticeVO> getNoticeListLately(int startIndexNo, int pageSize, int lately) {
			List<NoticeVO> vos = new ArrayList<NoticeVO>();
			try {
				sql = "select * from notice where date_sub(now(), interval ? day) > wDate order by idx desc limit ?,?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, lately);
		...
		// 조회수 1 증가처리
			...
			public int setNoticeUpdateOk(NoticeVO vo) {
				int res = 0;
				try {
					sql = "update notice set title=?,content=?,hostIp=? where idx=?";
					pstmt = conn.prepareStatement(sql);
			...
			public List<NoticeVO> getNoticeSearch(String search, String searchString, int startIndexNo, int pageSize) {
				List<NoticeVO> vos = new ArrayList<NoticeVO>();
				try {
					sql = "select * from notice where "+search+" like ? order by idx desc limit ?,?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, "%"+searchString+"%");
					pstmt.setInt(2, startIndexNo);
					pstmt.setInt(3, pageSize);
